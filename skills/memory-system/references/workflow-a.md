...
## A1.5：检测已有数据
检查用户选择的路径下是否存在已有记忆系统文件：
- 检测以下文件至少有一个存在：
  - `$MEMORY_DATA/USER/core.md`
  - `$MEMORY_DATA/USER/cognitive.md`
  - `$MEMORY_DATA/AGENTS/GL1-rules.md`
- **发现已有数据** → 跳转到 A6（不创建目录、不覆盖题库、不做问卷）
  - 告知用户："检测到已有记忆数据，跳过安装步骤，仅重建配置"
- **空目录** → 继续 A2（全新安装）
