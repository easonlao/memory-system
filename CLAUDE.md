...
Agent-Memory/ 源文件 ← 编辑
    ↓ 手动同步
skills/memory-installer/references/ ← 分发副本
```
...
1. 更新 `skills/memory-installer/metadata.json` 中 `version` 字段
...
```bash
# 同步单个文件到分发目录
cp Agent-Memory/AGENTS.md skills/memory-installer/references/

# 查看当前版本
cat skills/memory-installer/metadata.json | grep version

# 存档旧版本
mv archived/v<旧版本>/ .

# 对比源文件和分发副本差异
diff Agent-Memory/07-问卷.md skills/memory-installer/references/questionnaire.md
```
